  ...
  const SandboxSignupForm = ({ config }) => {
    ...
    const fetchNonceForForm = async () => {
      try {
        ...
        const freshNonce = await fetchFreshNonce("sandbox", moneybagAjax.ajaxurl);
        setNonce(freshNonce);
      } catch (error) {
        console.error("Failed to load security token");
        setErrors({
          general: "Failed to initialize form. Please refresh the page.",
        });
      } finally {
        ...
        if (digit === ":") {
          ...
          if (nonceLoading) {
            return createElement(
              "div",
              { className: "moneybag-loading" },
              createElement("p", null, "Loading form..."),
            );
          }
    ...
    const resendOTP = async () => {
      ...
      try {
        ...
        setErrors((prev) => ({
          ...prev,
          otp: `âœ“ Verification code sent successfully to ${formData.identifier}`,
        }));
        ...
        createElement(
          ...
          createElement(
            ...
            createElement(
              "a",
              {
                href: "https://demo.moneybag.com.bd/",
                target: "_blank",
                rel: "noopener noreferrer",
              ...
              createElement(
                ...
                errors.otp &&
                  createElement(
                    "span",
                    {
                      className: errors.otp.startsWith("âœ“")
