  ...
  const PricingPlanForm = ({ config }) => {
    ...
    useEffect(() => {
      const loadPricingRules = async () => {
        try {
          ...
          const pluginUrl =
            window.moneybagPricingAjax?.pluginUrl ||
            `${window.location.origin}/wp-content/plugins/moneybag-wordpress-plugin`;
    ...
    useEffect(() => {
      if (pricingRules && !formData.businessCategory) {
        ...
        const firstBusinessCategory =
          Object.keys(pricingRules.businessCategories || {})[0] || "";
    ...
    useEffect(() => {
      if (pricingRules && formData.businessCategory) {
        ...
        if (
          availableLegalIdentities.length > 0 &&
          !availableLegalIdentities.includes(formData.legalIdentity)
        ) {
          setFormData((prev) => ({
            ...prev,
    ...
    const loadRecaptcha = useCallback(() => {
      // Check if site key is in config (from PHP)
      if (!config.recaptcha_site_key) {
        return; // No site key, so don't load
      }
    ...
    const executeRecaptcha = useCallback(() => {
      return new Promise((resolve, reject) => {
        ...
        // Check if grecaptcha is loaded (using state)
          ...
          try {
            window.grecaptcha
              .execute(config.recaptcha_site_key, {
                action: "pricing_plan", // Specific action name
              })
              .then((token) => {
    ...
    const calculatePricingAndDocuments = () => {
      ...
      const businessCategoryData =
        pricingRules.businessCategories?.[formData.businessCategory];
      const legalIdentityData =
        businessCategoryData?.identities?.[formData.legalIdentity];
      ...
      const pricingByService = serviceTypes.map((service) => {
        ...
        if (pricingCategory) {
          ...
          const categoryPricing =
            pricingCategory[formData.businessCategory] ||
    ...
    // Modified to accept the token
    const createCRMEntries = async (token) => {
      try {
    ...
    const handleSubmit = async () => {
      ...
      // 1. Execute your Recaptcha function and get the token
      const token = await executeRecaptcha();

      // 2. Check if Recaptcha failed (and was required)
      if (!token && config.recaptcha_site_key) {
        setLoading(false);
        setErrors((prev) => ({
          ...prev,
          submit: "Recaptcha challenge failed. Please try again.",
        }));
      ...
      try {
        // 3. Pass the token to createCRMEntries
        const crmResult = await createCRMEntries(token);
        ...
        if (errorMessage.toLowerCase().includes("email")) {
          ...
          setErrors((prev) => ({
            ...prev,
            submit:
              "This information already exists in our system. Our team will contact you soon.",
          }));
        } else {
    ...
    const renderSelect = (name, options, placeholder = "Select") => {
      ...
      if (name === "businessCategory") {
        ...
        if (pricingRules && pricingRules.businessCategories) {
          fieldOptions = Object.keys(pricingRules.businessCategories).map(
            (category) => ({
              value: category,
              label: category,
            })
        ...
        if (
          pricingRules &&
          pricingRules.businessCategories &&
    ...
    if (currentStep === 1) {
      ...
      if (!pricingRules) {
        return createElement(
          "div",
          { className: "pricing-plan-container moneybag-form" },
          createElement(
            "div",
    ...
    if (currentStep === 2) {
      return createElement(
        ...
        createElement(
          "div",
          { className: "step-container-three-col" },
          createElement(
            "div",
            { className: "form-section" },
            createElement("h1", null, config.form_title),
            ...
            selectedPricing &&
              createElement(
                ...
                createElement(
                  ...
                  showAllPricing &&
                    (selectedPricing.services || []).slice(4).map((service) =>
                      createElement(
                        "div",
                        {
                          key: service.key,
                ...
                selectedPricing.services &&
                  selectedPricing.services.length > 4 &&
                  createElement(
                    "button",
                    {
    ...
    if (currentStep === 3) {
      return createElement(
        ...
        createElement(
          ...
          createElement(
            "div",
            { className: "consultation-image-container" },
            createElement("img", {
            ...
            "a",
            {
              ...
              onMouseEnter: (e) => {
                e.target.style.background =
                  "linear-gradient(270deg, #E45050 0%, #CC1A25 100%)";
                e.target.style.transform = "translateY(-2px)";
                e.target.style.boxShadow = "0 6px 20px rgba(238, 33, 46, 0.4)";
              },
              onMouseLeave: (e) => {
                e.target.style.background =
                  "linear-gradient(270deg, #F46F6F 0%, #EE212E 100%)";
